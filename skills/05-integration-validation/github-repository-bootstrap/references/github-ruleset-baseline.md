...
2. required check 的 context、`integration_id` 和 strict 状态与计划一致。
3. `bypass_actors` 与用户选择一致，并报告当前身份是否可旁路。
4. 默认分支有效规则包含相同 required check，且分支元数据显示 `protected=true`。
5. 仓库元数据返回 `delete_branch_on_merge=true`。
